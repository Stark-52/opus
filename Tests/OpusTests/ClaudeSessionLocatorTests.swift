...
final class ClaudeSessionLocatorTests: XCTestCase {
    ...
    func testEncodesSlashesToDashes() {
        let candidates = ClaudeSessionLocator.projectDirNameCandidates(
            for: "/Users/dev/Documents/GitHub/ClaudeUltra")
        XCTAssertEqual(candidates.first, "-Users-dev-Documents-GitHub-ClaudeUltra")
    }

    func testDottedPathYieldsBothEncodings() {
        let candidates = ClaudeSessionLocator.projectDirNameCandidates(
            for: "/Users/dev/.claude/worktrees/x")
        XCTAssertEqual(candidates, [
            "-Users-dev--claude-worktrees-x",   // current: all non-alnum → dash
            "-Users-dev-.claude-worktrees-x"    // older: dots kept
        ])
    }

    func testNoDuplicateCandidatesWhenPathHasNoDots() {
        let candidates = ClaudeSessionLocator.projectDirNameCandidates(for: "/Users/dev/proj")
        XCTAssertEqual(candidates, ["-Users-dev-proj"])
    }

    func testPicksMostRecentSession() throws {
        try makeProject("-Users-dev-proj", sessions: [
        ...
        let id = ClaudeSessionLocator.mostRecentSessionId(for: "/Users/dev/proj", projectsRoot: root)
    ...
    func testFallsBackToOlderDotKeepingDirName() throws {
        ...
        // (second candidate: "-Users-dev-.claude-x").
        try makeProject("-Users-dev-.claude-x", sessions: [(id: "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", age: 60)])
        let id = ClaudeSessionLocator.mostRecentSessionId(for: "/Users/dev/.claude/x", projectsRoot: root)
    ...
    func testRejectsNonUuidFilenames() throws {
        ...
        try makeProject("-Users-dev-proj2", sessions: [
            (id: "not-a-uuid", age: 60),
            (id: "44444444-aaaa-bbbb-cccc-000000000004", age: 3600)
        ])
        let id = ClaudeSessionLocator.mostRecentSessionId(for: "/Users/dev/proj2", projectsRoot: root)
    ...
    func testCurrentEncodingWinsOverOlder() throws {
        ...
        try makeProject("-Users-dev--claude-y", sessions: [
            (id: "55555555-aaaa-bbbb-cccc-000000000005", age: 60)
        ])
        try makeProject("-Users-dev-.claude-y", sessions: [
            (id: "66666666-aaaa-bbbb-cccc-000000000006", age: 60)
        ])
        let id = ClaudeSessionLocator.mostRecentSessionId(for: "/Users/dev/.claude/y", projectsRoot: root)
    ...
    func testReturnsNilWhenDirHasNoSessions() throws {
        try makeProject("-Users-dev-empty", sessions: [])
        XCTAssertNil(ClaudeSessionLocator.mostRecentSessionId(for: "/Users/dev/empty", projectsRoot: root))
    }
}
