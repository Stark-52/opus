...
- **Custom command execution.** Opus shells out to `/bin/zsh -i -c "<command>"` where `<command>` is constructed from the user-configured initial command + working directory. The custom-command field in Settings is *executed*, not sandboxed. Any process with write access to the Opus preferences plist (`~/Library/Preferences/com.stark52.opus.plist`) can cause Opus to run arbitrary commands at next launch.
